...
if(playerSide == west) then
{
	[] spawn
	{
		...
		while{true} do
		{
			...
			{
				case("U_Competitor"): {
					player setobjecttextureglobal [0,"textures\playerskins\cadet.paa"];
				};
				case("U_B_HeliPilotCoveralls"): {
					...
					player setobjecttextureglobal [0,"textures\playerskins\trained.paa"];
					};
					if((str(player) in ["cop_16","cop_17"])) then {
					player setobjecttextureglobal [0,"textures\playerskins\highway.paa"];;
					};
				};
			};
			_uni = uniform player;
