/*
	...
	@Edited: 30.01.2014
*/
if(isServer) exitWith {systemChat "BTSGAMING.NET :: INITIALIZING CLOSED DUE TO BEING SERVER"};
waitUntil {!isnull player && player == player};
//waitUntil {!isNil {life_fnc_displayHint}};
systemChat "BTSGAMING.NET :: INITIATING ANTI-CHEAT";
...
[] spawn {
	...
	"hub","scrollinit","gfYJV","Lystic_LMAOOOOOOOOOOOOOOOOOOO","Lystic_Re","Lysto_Lyst","Menulocations","Lystic_Init","scroll_m_init_star","ASSPLUNGE","exstr1","pathtoscrdir3","Monky_funcs_inited",
	"Monky_initMenu","player_zombieCheck","godlol","playericons","abcdefGEH","wierdo","go_invisible_infiSTAR","serverObjectMonitor","HaxSmokeOn","BLABLA","O_Soldier_F","FOXBYPASS","POLICE_IN_HELICOPTA","igodokxtt","enamearr","initarr3","locdb","sCode","infAmmoIndex",
...
if (call (missionNamespace getVariable ["life_adminlevel", {0}]) < 1) then {
...
systemChat "BTSGAMING.NET :: ANTI-CHEAT INITIALIZED";

diag_log "::Life Client:: Loading stuff from the server...";
clientStarted = player;
publicVariableServer "clientStarted";
waitUntil { (!isNil {AHAH}) };
diag_log "::Life Client:: Stuff received from the server.";
